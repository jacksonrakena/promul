...
namespace Promul
{
    internal sealed class NetConnectAcceptPacket : ConnectPacketBase
    {
        ...
        public static NetConnectAcceptPacket? FromData(NetworkPacket packet)
        {
            ...
            var connectionId = BitConverter.ToInt64(packet.Data[1..]);

            //check connect num
            var connectionNumber = packet.Data[9];
            if (connectionNumber >= NetConstants.MaxConnectionNumber)
                return null;

            //check reused flag
            var isReused = packet.Data[10];
            if (isReused > 1)
                return null;

            //get remote peer id
            var peerId = BitConverter.ToInt32(packet.Data[11..]);
            if (peerId < 0)
                return null;
